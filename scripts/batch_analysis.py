...
from src.file_handling import fname, load_tiff, save_dual_movie, save_img, write_tiff
from src.visualization import plot_corr_heatmap, plot_ccf, plot_aligned, plot_corrmap
...
def spatial_corrmaps(params):
    ...
    for p_tif in p_tifs:
        ...
        df = pd.read_parquet(fname(p_tif, 'data_ch1.parquet', new_root=p_out)) #either of the data files work
        ...
        for col in cols:
            plot_corrmap(ch1, ch2, df, col, f_ca=f_ca, f_beh=f_beh, path=p_plot / f'{col}_1xy.svg')
...
def pooled(params, p_tifs):
    ...
    l = []
    for p_tif in ps:
        df = pd.read_parquet(fname(p_tif, 'data_ch1.parquet', new_root=p_out)) #either of the data works
        l.append(df)
